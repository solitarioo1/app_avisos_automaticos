...
def api_exportar_csv_aviso(numero):
    ...
    try:
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cursor.execute(f"""
            SELECT cliente_id, nombre, apellido, dni_ruc, telefono, correo, nivel,
                   departamento, provincia, distrito, hectareas, monto_asegurado,
                   entidad_nombre, cultivo
            FROM v_clientes_por_aviso_completo
            WHERE {' AND '.join(condiciones)}
            ORDER BY CASE LOWER(nivel)
                WHEN 'rojo' THEN 1 WHEN 'naranja' THEN 2 WHEN 'amarillo' THEN 3 ELSE 4
        ...
        writer.writerow(['ID', 'Nombre', 'Apellido', 'DNI/RUC', 'Telefono', 'Correo', 'Nivel de Aviso',
                          'Departamento', 'Provincia', 'Distrito', 'Hectareas', 'Monto Asegurado',
                          'Entidad', 'Cultivo'])
        for r in rows:
            writer.writerow([
                r['cliente_id'], r.get('nombre', ''), r.get('apellido', ''), r.get('dni_ruc', ''),
        ...
        resp.headers['Content-Disposition'] = f'attachment; filename="clientes_aviso_{numero}.csv"'
        return resp
    except Exception as e:
        logger.error("Error exportando CSV de aviso %s: %s", numero, str(e))
        return jsonify({'error': str(e)}), 500